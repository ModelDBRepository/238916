...
public class GammaDist { //from "Algorithm from Numerical Recipes in C", Second Ed.
    ...
    java.util.Arrays.sort(realData,1,length+1);
          if (realMin == realMax) {
           ...
           System.out.println("GammaDist: GetCurveType: \t");
          if ( (realMin >= realMean) || (realMax <= realMean)) {
               System.out.println(
                      "mean in  function getCurveType is outside range, possible infinite loop.  Min, Mean, Max, Length, RealData: " +
                      realMin + "  " + realMean + "  " + realMax + " "+length+" "+
                    		  java.util.Arrays.toString(realData));
               System.exit(11);
          }
